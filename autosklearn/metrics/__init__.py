...
def statistical_parity_difference(y_true, y_pred, protected_attr, privileged_group=1):
    dataset = BinaryLabelDataset(df=pd.DataFrame({'label': y_true, 'protected': protected_attr}),
                                 label_names=['label'],
                                 protected_attribute_names=['protected'])
    pred_dataset = dataset.copy()
    pred_dataset.labels = np.array(y_pred).reshape(-1, 1)
    metric = ClassificationMetric(dataset, pred_dataset, unprivileged_groups=[{'protected': 1 - privileged_group}],
                                  privileged_groups=[{'protected': privileged_group}])
    return metric.statistical_parity_difference()

def disparate_impact(y_true, y_pred, protected_attr, privileged_group=1):
    dataset = BinaryLabelDataset(df=pd.DataFrame({'label': y_true, 'protected': protected_attr}),
                                 label_names=['label'],
                                 protected_attribute_names=['protected'])
    pred_dataset = dataset.copy()
...
def equal_opportunity_difference(y_true, y_pred, protected_attr, privileged_group=1):
    dataset = BinaryLabelDataset(df=pd.DataFrame({'label': y_true, 'protected': protected_attr}),
                                 label_names=['label'],
                                 protected_attribute_names=['protected'])
    pred_dataset = dataset.copy()
...
def average_odds_difference(y_true, y_pred, protected_attr, privileged_group=1):
    dataset = BinaryLabelDataset(df=pd.DataFrame({'label': y_true, 'protected': protected_attr}),
                                 label_names=['label'],
                                 protected_attribute_names=['protected'])
    pred_dataset = dataset.copy()
...
# Auto-Sklearn Scorers
spd_scorer = make_scorer(statistical_parity_difference)
di_scorer = make_scorer(disparate_impact)
eod_scorer = make_scorer(equal_opportunity_difference)
aod_scorer = make_scorer(average_odds_difference)
...
CLASSIFICATION_METRICS.update({
    "statistical_parity_difference": spd_scorer,
    "disparate_impact": di_scorer,
    "equal_opportunity_difference": eod_scorer,
    "average_odds_difference": aod_scorer,
})
