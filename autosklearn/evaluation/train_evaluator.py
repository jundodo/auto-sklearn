...
def compute_fairness_metrics(y_true, y_pred, protected_attribute):
    privileged = (protected_attribute == 1)  # 비보호 그룹 (남성 등)
    unprivileged = (protected_attribute == 0)  # 보호 그룹 (여성 등)

    # 긍정적 결과 비율 (P(Y=1 | A=0), P(Y=1 | A=1))
    p_unprivileged = np.mean(y_pred[unprivileged] == 1)
    p_privileged = np.mean(y_pred[privileged] == 1)

    # SPD (Statistical Parity Difference)
    spd = p_unprivileged - p_privileged

    # DI (Disparate Impact) - 1에 가까울수록 좋음
    di = p_unprivileged / (p_privileged + 1e-10)  # 0으로 나누는 문제 방지

    # True Positive Rate (TPR) 계산
    tpr_unprivileged = np.sum((y_pred[unprivileged] == 1) & (y_true[unprivileged] == 1)) / \
                       (np.sum(y_true[unprivileged] == 1) + 1e-10)

    tpr_privileged = np.sum((y_pred[privileged] == 1) & (y_true[privileged] == 1)) / \
                     (np.sum(y_true[privileged] == 1) + 1e-10)

    # Equal Opportunity Difference (EOD)
    eod = tpr_unprivileged - tpr_privileged

    # False Positive Rate (FPR) 계산
    fpr_unprivileged = np.sum((y_pred[unprivileged] == 1) & (y_true[unprivileged] == 0)) / \
                       (np.sum(y_true[unprivileged] == 0) + 1e-10)

    fpr_privileged = np.sum((y_pred[privileged] == 1) & (y_true[privileged] == 0)) / \
                     (np.sum(y_true[privileged] == 0) + 1e-10)

    # Average Odds Difference (AOD)
    aod = ((tpr_unprivileged - tpr_privileged) + (fpr_unprivileged - fpr_privileged)) / 2
...
def apply_reject_option_classifier(y_pred, y_true, protected_attribute, metric_name):

    roc = RejectOptionClassification(metric_name=metric_name)
    roc.fit(y_true, y_pred, protected_attribute)
    y_pred_adjusted = roc.predict(y_pred)
...
def fairness_cost_function(accuracy, fairness_metrics, beta=0.5, lambda_=1.0, a_baseline=0.1):
    fairness_score = np.mean(list(fairness_metrics.values()))  # 여러 fairness 지표 평균값 사용
    fairness_penalty = lambda_ * max(0, a_baseline - 0.9 * fairness_score)
    cost = beta * accuracy + (1 - beta) * (1 - fairness_score) * fairness_penalty
    return cost
...
class TrainEvaluator(AbstractEvaluator):
    ...
    def fit_predict_and_loss(self, iterative: bool = False) -> None:
        ...
        accuracy_loss = self._loss(self.Y_targets, self.Y_optimization_pred)
        fairness_metrics = compute_fairness_metrics(
            y_true=self.Y_targets,
            y_pred=self.Y_optimization_pred,
            protected_attribute=protected_attribute
        )
        fairness_loss = fairness_cost_function(accuracy_loss, fairness_metrics)
        ...
        if iterative:
            ...
            self.finish_up(
                ...
                additional_run_info={"fairness_metrics": fairness_metrics},
                file_output=True,
                final_call=True,
                status=status,
            )
